...
function getModelFileStart(indentation, spaces, tableName) {
  ...
  fileStart += 'import {' + tableName + 'Model} from \'../../typings/app/model/db\';\n\n';
  fileStart += "export default (app:Application) => {\n";
  fileStart += spaces + "const DataTypes = app.Sequelize;\n\n";
  fileStart += spaces + 'const Model = <' + tableName + 'Model>app.model.define(\'' + tableName + '\', {\n';
  return fileStart;
}
...
function getTableDefinition(tsTableDefAttr, tableName) {
  ...
  tableDef += 'export type ' + tableName + 'Model = typeof ' + tableName + 'Attribute & { new (values?: object, options?: BuildOptions): ' + tableName + 'Attribute;}'
  return tableDef;
}
