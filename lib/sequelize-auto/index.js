...
AutoSequelize.prototype.write = function(attributes, typescriptFiles, callback) {
  ...
  var self = this;
  const db_d_ts = './typings/app/model/'

  mkdirp.sync(path.resolve(self.options.directory));
  mkdirp.sync(path.resolve(db_d_ts));

  async.each(tables, createFile, callback);

  if (self.options.typescript) {
    if (typescriptFiles !== null && typescriptFiles.length > 1) {
      fs.writeFileSync(path.join(db_d_ts, 'db.d.ts'), typescriptFiles[0], 'utf8');
      fs.writeFileSync(path.join(db_d_ts, 'db.tables.ts'), typescriptFiles[1], 'utf8');
    }
  }
